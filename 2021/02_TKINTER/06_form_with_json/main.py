from tkinter import Tk, Entry, Label, Entry, Button, StringVar, Frame, Radiobutton
...
def save_btn():
	#print(name_var.get(), origin_var.get(), birth_place_var.get())
	#print(day_var.get(), month_var.get(), year_var.get(), jk_var.get())
	name, origin, birth_place = name_var.get(), origin_var.get(), birth_place_var.get()
	name_entry.focus()
	if name and origin and birth_place:
		student = {
			"name":name, 
			"origin":origin,
		...
		origin_var.set("")
		birth_place_var.set("")
		msg_label.configure(text="Data saved", fg="green")
		return True
	msg_label.configure(text="Pastikan data telah diisi", fg="red")
	return False
window = Tk()
...
msg_label.grid(row=1, column=0, columnspan=2, sticky="nswe")
...
birth_date_frame.grid(row=5, column=1, sticky="nsew")
...
year_combobox['value'] = list(range(2000, 2022))
...
jk_label.grid(row=6, column=0, sticky="w")

jk_frame = Frame(window)
jk_frame.grid(row=6, column=1, sticky="nsew")
...
save_btn = Button(window, text="save data", command=save_btn)
